...
module.exports = {
    ...
    hasCmdAccess: function (message) {
        return (
            ...
            Admin.users.includes(message.author.id) ||
            Admin.channels.includes(message.channel.id)
        )
    },
    ...
    listAccessCMD: {
        ...
        full_descrip:
            'Every command may be used by the people listed and from the channels listed. To give access or remove it use `$addCmdAccess` or `$removeCmdAccess` respectively. The owners are hardcoded into the bot and their access cannot be changed',
        hidden: true,
    ...
    addCmdAccessCMD: {
        ...
        function: async function (bot, msg, args) {
            ...
            if (args.length == 0 && !Admin.channels.includes(msg.channel.id)) {
                // add current channel
    ...
    listBansCMD: {
        ...
        full_descrip:
            'Users listed are not allowed to submit to the competition. To add a ban or remove one use `$ban` or `$unban` respectively',
        hidden: true,
        function: async function (bot, msg, args) {
            if (!module.exports.hasCmdAccess(msg)) return
    ...
    BanCMD: {
        ...
        function: async function (bot, msg, args) {
            ...
            if (Bans.includes(id))
                return `${user.username} \`(${id})\` is already banned`
            module.exports.addBan(id)

            args.shift()
            var reason = args.length
                ? 'Provided Reason: ' + args.join(' ')
