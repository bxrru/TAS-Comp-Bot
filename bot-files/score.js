...
module.exports = {
    ...
    alg: function (k, n) {
        var points = [
            -1, 600, 569, 539, 510, 482, 455, 429, 404, 380, 357, 335, 314, 294,
            275, 257, 240, 224, 209, 195, 182, 170, 159, 149, 140, 132, 125,
            119, 114, 110, 107, 105, 104,
        ]
    ...
    resultsToScore: function (a) {
        ...
        for (var i = a.length - 1; i >= 0; i--) {
            ...
            if (
                a[i] == '' ||
                a[i].toUpperCase().substr(0, 2) == 'DQ' ||
                a[i].toUpperCase().substr(0, 4) == 'HTTP'
            ) {
                a.splice(i, 1)
            }
        }
    ...
    addScores: function (a, b) {
        if (a === undefined || a.length == 0 || a[0].length == 0) {
            return b
        }
        if (b === undefined || b.length == 0 || b[0].length == 0) {
            return a
        }
    ...
    updateScore: function (resultsMessage) {
        ...
        try {
            ...
            if (this.getTask() % this.getSetLength() == 0) {
                this.setScore([])
            }
    ...
    combine: function (name1, name2) {
        ...
        if (name1 == name2) {
            return 'Cannot combine the same name.'
        }
                ...
                for (var j = 0; j < score.length; j++) {
                    if (score[j][1].toUpperCase() == name2) {
                        score[i][2] = (
                            ...
                            parseFloat(score.splice(j, 1)[0][2])
                        ).toFixed(1)
                        score = this.sortScore(score)
                        this.setScore(score)
            ...
            case 'SETFEED':
                if (args.length == 0) {
                    ...
                    if (args.length == 0) {
                        ...
                        msg =
                            "Argument not recognized. Specify `'results'` or 'score'"
                    }
                }
                break
    ...
    processCommand: async function (bot, msg, args) {
        ...
        if (['FIND', 'CALCULATE'].includes(action) || users.hasCmdAccess(msg)) {
            ...
            if (action == 'SETMESSAGE') {
                if (params.length >= 2) {
                    bot.getMessage(params[0], params[1])
                        .then((msg) => {
            ...
            if (
                ...
                try {
                    await bot.editMessage(
                        this.getScoreMsg()[0],
                        this.getScoreMsg()[1],
                        this.scoreToMessage(this.getScore(), this.getTask())
                    )
                } catch (e) {
                    this.setScoreMsg('', '')
            ...
            if (action == 'INFO') {
                try {
                    ...
                    message +=
                        '**Score Message URL** - https://discordapp.com/channels/'
                    message +=
                        scoremsg.channel.guild.id +
                        '/' +
                        scoremsg.channel.id +
                        '/' +
                        scoremsg.id
    ...
    help: function (command) {
        ...
        msg +=
            '```$score calculate *<length of previous score> *<previous scores...> *<results...>```'
