const users = require("./users.js"),
      miscfuncs = require("./miscfuncs.js"),
      Save = require("./save.js");

// 35 default emoji
let DefaultEmoji = ["smiley", "smile", "grin", "sweat_smile", "joy", "innocent", "slight_smile", "upside_down", "wink", "heart_eyes", "kissing_heart", "stuck_out_tongue", "smirk", "sunglasses", "pensive", "weary", "cry", "rage", "flushed", "scream", "thinking", "grimacing", "frowning", "open_mouth", "cowboy", "smiling_imp", "clown", "robot", "thumbsup", "thumbsdown", "clap", "ok_hand", "wave", "pray", "eyes"],
    DisabledServers = [],
    BoundSlots = true,
    Rigged_Ids = [];
...
module.exports = {
    ...
    load: () => {
        let data = Save.readObject("games.json");
        DisabledServers = [];
    ...
    slots: {
        ...
        function: (bot, msg, args) => {
            ...
            const numEmoji = Math.min(Math.max(parseInt(args[0]) || 3, 2), 10000), // Sets maximum number of emojis to 10000 as to not go on forever if it is unbound
                  defaultEmoji = miscfuncs.isDM(msg) || msg.channel.guild.emojis.length === 0,
                  emojis = defaultEmoji ? DefaultEmoji : msg.channel.guild.emojis,
                  rigged = Rigged_Ids.includes(msg.author.id) ? randInt(emojis.length) : -1;
    ...
    spin: {
        ...
        function: (bot, msg, args) => {
            ...
            let emojis = (miscfuncs.isDM(msg) || msg.channel.guild.emojis.length === 0) ? DefaultEmoji : msg.channel.guild.emojis,
                roll = [],
                win = false,
                result = "";
    ...
    assign_random: {
        ...
        function: async (bot, msg, args) => {
            ...
            for (let i = 0, j = users.length; i < j; i++) {
                let dm = await bot.getDMChannel(users[i].id).catch(e => "DM Failed ``" + e + "``");
