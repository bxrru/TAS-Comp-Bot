...
module.exports = {
    ...
    toggle: {
        ...
        full_descrip:
            'Switches the game functions on/off for a specific server',
        hidden: true,
    ...
    bound: {
        ...
        full_descrip:
            "A global switch to bound/unbound slots. Bounded means that it limits $slots to 1 message. If it's unbounded it will send as many messages as it takes to send all the requested emoji (**WARNING** Walls of emoji may cause discord to lag)",
        hidden: true,
        function: (bot, msg, args) => {
            if (!users.hasCmdAccess(msg)) return
    ...
    giveaway: {
        ...
        full_descrip:
            'Randomly selects a winner from arguments for a giveaway.',
        hidden: true,
        function: (bot, msg, args) => {
    ...
    slots: {
        ...
        full_descrip:
            "Chooses a number of random emojis. This number is specified by the user and defaults to 3. The limit is 10000 emojis, or as many characters that can fit in a message if slots is bounded. This will use the server's custom emoji. If it has none, or this is used in DMs this will use a selection of 35 default emoji",
        hidden: true,
        function: (bot, msg, args) => {
            ...
            const defaultEmoji =
                miscfuncs.isDM(msg) || msg.channel.guild.emojis.length === 0
            const emojis = defaultEmoji
                ? DefaultEmoji
                : msg.channel.guild.emojis
            const rigged = Rigged_Ids.includes(msg.author.id)
                ? randInt(emojis.length)
                : -1
            ...
            for (let i = 0; i < numEmoji - 1; i++) {
                ...
                if (defaultEmoji ? last !== emoji : last !== emoji.id)
                    win = false
            ...
            bot.createMessage(
                msg.channel.id,
                win ? `WINNER! ${msg.author.mention}` : 'Please Play Again'
            )
        },
    },
    rig: {
        ...
        function: (bot, msg, args) => {
            ...
            for (let i = 0; i < Rigged_Ids.length; i++)
    ...
    spin: {
        ...
        full_descrip:
            "Spin the classic slot machine to choose n^2 (9 by default, 49 at most) random emoji. Win by getting n in a row in any row, column, or diagonal! This will use the server's custom emoji. If it has none, or this is used in DMs this will use a selection of 35 default emoji",
        hidden: true,
        function: (bot, msg, args) => {
            if (!miscfuncs.isDM(msg) && disabled(msg.channel.guild.id)) return
            ...
            for (let i = 0; i < dim; i++) {
                // if (rows || columns)
                if (
                    new Set(roll.slice(dim * i, dim * (i + 1))).size === 1 ||
                    new Set(roll.filter((_, j) => j % dim === i)).size === 1
                ) {
                    win = true
                    break
                }
            ...
            if (
                !win &&
                (new Set(roll.filter((_, i) => i % (dim + 1) === 0)).size ===
                    1 ||
                    new Set(
                        roll.filter(
                            (_, i) =>
                                i > 0 && i < dim ** 2 - 1 && i % (dim - 1) === 0
                        )
                    ).size === 1)
            )
                win = true
    ...
    assign_random: {
        ...
        full_descrip:
            'Sends a DM to everyone @ mentioned in the command, with a name of another person on the list. No 2 users will get each other, and nobody will get themself.',
        hidden: true,
