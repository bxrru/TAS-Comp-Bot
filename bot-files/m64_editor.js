    ...
    encode: {
        ...
        function: async (bot, msg, args) => {
            ...
            const mupen_args = ["-m64", `${process.cwd() + save.getSavePath().substring(1) }/tas.m64`, "-avi", "encode.avi", "-lua", LUA_INPUTS],
                ffmpeg_args = {vcodec: "libx264", acodec: "libopus", vrate: "", crf: "", arate: "128", clamp: false};
                    ...
                    try {
                        ...
                        if (!ffmpeg_args.clamp) {
                            ...
                            const arate = Math.min(16*Math.floor(trate / 128), 128), // Multiple of 16kb/s between 0 and 1024kb/s total rate
                                vrate = (trate - arate) * 0.9; // Make up for header and vbv bitrate overshoot
