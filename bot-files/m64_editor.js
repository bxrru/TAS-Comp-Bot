...
const HEADER = {
    ...
    0x01c: [
        UInt,
        2,
        'Movie start type (from snapshot is 1, from power-on is 2)',
    ],
    0x01e: [UInt, 2, 'Reserved (0)'],
    0x020: [
        BitField,
        4,
        'Controllers (from least to most significant, the bits are for controllers 1-4: present, has mempack, has rumblepak)',
    ],
    //0x024: [UInt, 160, `Reserved (0)`],
    0x0c4: [
        AsciiString,
        32,
        'internal name of ROM used when recording (directly from ROM)',
    ],
...
function bufferToString(buffer, encoding = 'utf8') {
    while (
        ...
        buffer[Buffer.byteLength(buffer) - 1] == 0
    ) {
...
function fix_controllers(m64) {
    ...
    for (let i = 0x400; i < m64.length; i += 0x4 * num_controllers) {
        // copy controller 1 data
...
function NextProcess(bot, retry = true) {
    ...
    let preprocessTAS = () => {
        ...
        if (crc in KNOWN_CRC == false) {
            // what if the user is the bot (internal calls?)
            if (crc == '') {
                ...
                if (retry) {
                    ...
                    console.log(
                        `ERROR: double empty CRC when running Mupen\n${JSON.stringify(request)}`
                    )
                } else {
                    bot.createMessage(
                        request.channel_id,
                        'Error: Failed to read CRC twice (could not read file?). Please ensure that links to files are not expired.'
                    )
                }
            } else if (request.channel_id == null) {
                console.log(
            ...
            bot.createMessage(
                request.channel_id,
                `<@${request.user_id}> Your encode was skipped: ghosts are only supported on the \`Super Mario 64 (USA)\` ROM.`
            )
            MupenQueue.shift()
            NextProcess(bot)
            return
        ...
        let runMupen = async () => {
            ...
            Mupen.on('close', async (code, signal) => {
                if (fs.existsSync('TLE.txt')) {
                    // currently do nothing on time limit exceeded...
                    ...
                    await request.callback(
                        ...
                        m64
                    ) // pass true if the run timed out
                } else {
                    await request.callback(
                        false,
                        false,
        ...
        if (!request.st_url) {
            // no savestate given, skip downloading savestate
        ...
        downloadAndRun(undefined, runMupen, request.st_url, 'tas.st')
    }
    ...
    downloadAndRun(undefined, preprocessTAS, request.m64_url, 'tas.m64')
}
...
module.exports = {
    ...
    rerecords: {
        ...
        function: async function (bot, msg, args) {
            ...
            if (rerecords > MAX_RR) {
                ...
                bot.createMessage(
                    msg.channel.id,
                    'WARNING: Max rerecord count exceeded'
                )
            } else if (rerecords < 0) {
                rerecords = 0
                bot.createMessage(
                    msg.channel.id,
                    'WARNING: Min rerecord count exceeded'
                )
            }
    ...
    description: {
        ...
        function: async function (bot, msg, args) {
            ...
            while (Buffer.byteLength(descrip) < SIZE) {
                // force to fill
    ...
    author: {
        ...
        function: async function (bot, msg, args) {
            ...
            while (Buffer.byteLength(author) < SIZE) {
                // force to fill
    ...
    encode: {
        ...
        function: async (bot, msg, args) => {
                ...
                switch (args[0].toUpperCase()) {
                    case 'CANCEL':
                        for (let i = 0; i < MupenQueue.length; i++) {
                            if (
                                MupenQueue[i].user_id == msg.author.id &&
                                MupenQueue[i].process === null &&
                                !MupenQueue[i].skip
                            ) {
                    ...
                    case 'FORCESKIP':
                        if (
                            users.hasCmdAccess(msg) ||
                            msg.author.id === MupenQueue[0].user
                        ) {
            ...
            if (m64_url.length == 0 /* || !st_url*/)
                return 'Missing/Invalid Arguments: `$encode [cancel/forceskip/queue/nolua/maxvbitrate=<bitrate>/crf=<crf>/abitrate=<bitrate>/constrainsize] <m64> [st/savestate]`'
            m64_url = m64_url[0]
            ...
            for (const arg of args) {
                if (
                    ['NOLUA', 'NO-LUA', 'DISABLE-LUA', 'LUABEGONE!'].includes(
                        arg.toUpperCase()
                    )
                ) {
                    use_lua = false
                } else if (
                    ['H265', 'H.265', 'HEVC'].includes(arg.toUpperCase())
                ) {
                    ffmpeg_args.vcodec = 'libx265'
                } else if (
                    ['CLAMP', 'CONSTRAINSIZE'].includes(arg.toUpperCase())
                ) {
                    ffmpeg_args.clamp = true
                } else if (
                    ['TRANSPARENT', 'INVIS'].includes(arg.toUpperCase())
                ) {
                    ...
                    switch (values[0].toUpperCase()) {
                        ...
                        case 'M-B:V':
                            ...
                            if (values[1] !== values[1] || values[1] < 64)
                                return 'Invalid value for maximum video bitrate.'
                        ...
                        case 'B:A':
                            ...
                            if (
                                values[1] !== values[1] ||
                                (values[1] < 8 && values[1] !== 0)
                            )
                                return 'Invalid value for audio bitrate.'
            ...
            let queue_position = QueueAdd(
                ...
                async (TLE, MISMATCH_SETTINGS, start_time, m64) => {
                    ...
                    if (TLE) {
                        ...
                        result =
                            fs.readFileSync(LUAPATH + 'error.txt').toString() +
                    ...
                    await bot
                        .createMessage(msg.channel.id, result)
                        .catch(console.error)
                },
                msg.channel.id,
                msg.author.id,
