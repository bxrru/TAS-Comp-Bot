...
function chooseChannel(string) {
    ...
    if (CHANNELS[string]) {
        ...
        if (string.startsWith('<#') && string.endsWith('>'))
            string = string.substr(2, string.length - 3) // detect channel mentions
        return string
    }
}
...
module.exports = {
    ...
    getChannelAliases: {
        ...
        full_descrip:
            'Retrieves the list of channel aliases that may replace `<channel_id>` in other commands',
        hidden: true,
        function: function (bot, msg, args) {
            if (!users.hasCmdAccess(msg)) return
    ...
    send: {
        ...
        function: function (bot, msg, args) {
            ...
            bot.createMessage(chooseChannel(channel), args.join(' ')).catch(
                (e) => {
                    return e.toString()
                }
            )
    ...
    delete: {
        ...
        full_descrip:
            'Usage: `$delete <channel_id> <message_id>`\nFor a list of channel names that can be used instead of `<channel_id>` use `$lc`',
        hidden: true,
        function: function (bot, msg, args) {
            if (!users.hasCmdAccess(msg)) return

            bot.getMessage(chooseChannel(args[0]), args[1])
                .then((msg) => {
                    msg.delete()
                })
                .catch((e) => {
    ...
    pin: {
        ...
        full_descrip:
            'Usage: `$pin <channel_id> <message_id>`\nFor a list of channel names that can be used instead of `<channel_id>` use `$lc`',
        hidden: true,
        function: function (bot, msg, args) {
            if (!users.hasCmdAccess(msg)) return
    ...
    unpin: {
        ...
        full_descrip:
            'Usage: `$unpin <channel_id> <message_id>`\nFor a list of channel names that can be used instead of `<channel_id>` use `$lc`',
        hidden: true,
        function: function (bot, msg, args) {
            if (!users.hasCmdAccess(msg)) return
    ...
    dm: {
        ...
        function: async function (bot, msg, args) {
            ...
            let dm = await bot.getDMChannel(user_id).catch((e) => {
                return 'DM Failed ``' + e + '``'
            })
