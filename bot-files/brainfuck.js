...
var info =
    'Compiles brainfuck code \`+-<>.,[]\` (See https://en.wikipedia.org/wiki/Brainfuck).'
info +=
    'Code that asks for input with \`,\` will take the first character of the next message it receives.'
...
async function Input(bot, channel, silent) {
    ...
    if (!silent)
        channel.createMessage(
            `Input Received: ${recent_msg.content.charCodeAt(0)} (${recent_msg.content.substr(0, 1)})`
        )
