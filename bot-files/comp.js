...
let NamesFree = [
    ...
    'Alexpalix1',
    'tjk',
] // arbitrary starting selection
let NamesUsed = []
const NAMES_PER_ENTRANT = 3
function RandomName() {
    let name = '' + Math.floor(Math.random() * 1000000000) // use random numbers if we run out of names somehow
    if (NamesFree.length) {
        name = NamesFree.splice(
            Math.floor(Math.random() * NamesFree.length),
            1
        )[0]
    }
    for (let i = 0; i < Submissions.length; ++i) {
        if (Submissions[i].name == name) {
            // this name is being reused, so remove the submission
...
function notifyHosts(bot, message, prefix) {
    ...
    var dm = async function (id) {
        if (
            IgnoreUpdates[id] &&
            IgnoreUpdates[id].includes(prefix.toUpperCase())
        )
            return
        try {
...
function AutoTimeEntry(
    ...
    if (
        !fs.existsSync(filepath + '.m64') ||
        !fs.existsSync(filepath + st_ext)
    ) {
    ...
    return Mupen.Process(
        ...
        () => {
            ...
            if (fs.existsSync(LUAPATH + 'submission.m64'))
                fs.unlinkSync(LUAPATH + 'submission.m64')
            fs.copyFileSync(filepath + '.m64', LUAPATH + 'submission.m64')
            if (fs.existsSync(LUAPATH + 'submission.st'))
                fs.unlinkSync(LUAPATH + 'submission.st')
        ...
        async (TLE, MISMATCH_SETTINGS) => {
            ...
            if (MISMATCH_SETTINGS) {
                result =
                    "DQ Can't playback m64. Make sure you use 1 controller with rumblepak & mempak disabled!"
            } else {
                if (fs.existsSync(LUAPATH + 'result.txt')) {
                    result = fs.readFileSync(LUAPATH + 'result.txt').toString()
                } else {
                    result =
                        'DQ the timing script could not load, likely your m64 or st caused Mupen to crash.'
                }
            }
            var user_msg = ``
            var unchanged = false
            if (result.startsWith('DQ')) {
                ...
                LOG(
                    Submissions[getSubNum()].id,
                    `Time: ${frames}${info.length ? `[${info}]` : ''}`
                )
            }
            module.exports.save()
            if (fs.existsSync(LUAPATH + 'results.txt'))
            ...
            if (!unchanged || (unchanged && !err_channel_id)) {
                try {
                    ...
                    // [AF] issue timing old runs since the uid has '-' at the end
                    admin_msg += `**Warning:** Failed to notify user (\`${Submissions[getSubNum()].id}\`) of their time update. `
                }

                if (completedTeam(Submissions[getSubNum()].id)) {
                    // message teammate
                    dm = await bot.getDMChannel(
                        Teams[Submissions[getSubNum()].id]
                    )
                    dm.createMessage(user_msg.trim()).catch((e) => {
...
function CheckAutoTiming(bot, msg) {
    ...
    Submissions.forEach((submission, index) => {
        if (
            submission.id != msg.author.id &&
            !(msg.author.id in Teams && submission.id == Teams[msg.author.id])
        )
            return
...
async function submissionName(bot, user_id, only_team_name = false) {
    ...
    if (!completedTeam(user_id)) {
        if (user_id in Nicknames /* [AF] dont use nicknames */) {
            ...
            if (submitted.length) {
                // instead of loading the user with the bot
    ...
    try {
        ...
        if (user && partner) {
            ...
            if ([user_id, Teams[user_id]] in Teams) {
                ...
                return `${Teams[[user_id, Teams[user_id]]]} (${name1} & ${name2})`.substring(
                    0,
                    MAXNAMELEN
                )
            }
...
function is_required_ext(ext) {
    ...
    let includes_recur = function (arr, first = true) {
        ...
        if (first && found) {
            // recursively find first element
            found = arr[0]
...
async function forwardSubmission(bot, user, filename, url) {
    if (!module.exports.hasSubmitted(user.id))
        return console.log('SOMETHING WENT WRONG: COULD NOT FORWARD SUBMISSION')
    ...
    if (completedTeam(user.id)) {
        ...
        try {
            ...
            try {
                ...
                console.log(
                    'Failed to notify task entrant that their partner was failed to be notified of a submission'
                )
            }
        }
    }
...
async function storeFile(bot, msg, attachment_url, extension, allow_autotime) {
    ...
    let filepath =
        Save.getSavePath() + '/Submissions/' + msg.author.id + extension

    Save.downloadFromUrl(attachment_url, filepath, async () => {
        ...
        try {
            ...
            let submitter_notif =
                fileExt(file) +
                ' submitted. Use `$status` to check your submitted files. ' /* Your alias is " + name [AF]*/
            if (!AllowAutoTime)
                submitter_notif += 'Autotiming is currently disabled.'
            let message = await bot.createMessage(
                msg.channel.id,
                submitter_notif,
                file
            )
...
async function updateSubmissionMessage(bot, force_new = false) {
    ...
    if (
        (Channel_ID == '' || Message_IDs.length == 0) &&
        SubmissionsChannel != ''
    ) {
        updated = true
        ...
        try {
            ...
            notifyHosts(
                bot,
                `Failed to send submission update to <#${SubmissionsChannel}> \`(${SubmissionsChannel})\``
            )
        }
        ...
        try {
            for (let i = 0; i < msgs.length; ++i) {
                if (i < Message_IDs.length) {
                    let message = await bot.getMessage(
                        Channel_ID,
                        Message_IDs[i]
                    )
            ...
            while (Message_IDs.length > msgs.length) {
                // possibly someone removed from msg
            ...
            notifyHosts(
                bot,
                'Failed to edit submission message ```' + e + '```'
            )
        }
    }
    if (updated) {
...
async function filterFiles(bot, msg, attachment, allow_autotime) {
    ...
    if (!module.exports.hasSubmitted(msg.author.id)) {
        ...
        notifyHosts(
            bot,
            `${await submissionName(bot, msg.author.id)} (${Submissions.length}) \`(${msg.author.id})\``,
            `New Submission`
        )
    } /* else { // [AF] 
    ...
    if (RoleStyle == `ON-SUBMISSION`) {
        if (completedTeam(msg.author.id))
            module.exports.giveRole(
                bot,
                Teams[msg.author.id],
                msg.author.username + `'s Partner`
            )
...
module.exports = {
    ...
    allowSubmissions: {
        ...
        full_descrip:
            'Starts accepting submissions via DMs. To start a timed task where participants have a limited amount of time to submit use `$startTask timed`',
        hidden: true,
        function: function (bot, msg, args) {
            ...
            if (
                Submissions.length ||
                DQs.length ||
                TimedTaskStatus.completed.length ||
                TimedTaskStatus.started.length
            )
                notifyHosts(
                    bot,
                    'Preexisting submissions detected. Use `$clearsubmissions` to remove previous data',
                    'WARNING'
                )
            ...
            if (TimedTask && ReleaseDate > new Date()) {
                ...
                notifyHosts(
                    bot,
                    `The scheduled release date has already passed - no "Task is Live!" message will appear and submissions must be closed manually with \`$stoptask\`. `,
                    `WARNING`
                )
            }
    ...
    startTimedTask: function (bot) {
        ...
        try {
            ...
            notifyHosts(
                bot,
                `Failed to send \`Task is live!\` message \`\`\`${e}\`\`\``,
                `ERROR`
            )
        }
    },
    ...
    stopSubmissions: {
        ...
        function: async function (bot, msg, args) {
            ...
            if (
                Announcement.KillDelayedFunction(
                    `COMP-WARN ${TaskChannel}`,
                    true
                )
            ) {
            ...
            if (Announcement.KillDelayedFunction(`COMP-END`, true)) {
                // TimedTaskStatus.started.length
            ...
            if (args.length > 0 && TaskChannel != ``) {
                try {
                    bot.createMessage(
                        TaskChannel,
                        `Task ${Task} is complete! Thank you for participating!`
                    )
                } catch (e) {
    ...
    clearSubmissions: {
        ...
        short_descrip:
            'Deletes all submission files (WARNING: NO CONFIRMATION)',
        full_descrip:
            'Removes the Submitted role from every user that has submitted. Deletes the message containing all the submissions and deletes all of the saved files **without a confirmation/warning upon using the command**. This also removes everyone from the list of timed task participants. ',
        hidden: true,
        function: async function (bot, msg, args) {
            if (notAllowed(msg)) return
    ...
    manuallyAddSubmission: {
        ...
        function: async function (bot, msg, args) {
            ...
            notifyHosts(
                bot,
                `${name} (${Submissions.length}) [Added by ${msg.author.username}]`,
                `New Submission`
            )
    ...
    removeSubmission: {
        ...
        function: async function (bot, msg, args) {
            ...
            var deleted = num.dq
                ? DQs.splice(num.number - 1, 1)[0]
                : Submissions.splice(num.number - 1, 1)[0]
            ...
            if (SubmittedRole != '') {
                try {
                    await bot.removeGuildMemberRole(
                        ...
                        deleted.id,
                        SubmittedRole,
                        'Submission Deleted by ' + msg.author.username
                    )
                    result += 'Removed role'
                } catch (e) {
    ...
    setSubmissionFile: {
        ...
        function: async function (bot, msg, args) {
            ...
            if (Submissions.length == 0)
                return 'There are no submissions to edit'
            if (args.length < 2)
                return 'Not Enough Arguments: `<submission_number> <url>`'
            ...
            storeFile(
                bot,
                { author: user, channel: msg.channel },
                args[0],
                '.' + filetype,
                true
            )
    ...
    setRole: {
        ...
        function: async function (bot, msg, args) {
            ...
            var role = args[0]
            var reason =
                'Testing role permissions, command call by ' +
                msg.author.username
            ...
            try {
                ...
                result +=
                    'Invalid Role: Role \`' +
                    role +
                    '\`does not exist or does not match the server. '
                result +=
                    'Use `$setServer <id>` to set the server that contains the role. '
    ...
    setFeed: {
        ...
        aliases: ['setfeed'],
        short_descrip:
            'Sets the default channel to send the submissions list to',
        full_descrip:
        ...
        function: async function (bot, msg, args) {
            ...
            var result =
                'Default submissions channel changed from <#' +
                SubmissionsChannel +
                '> to <#' +
                channel +
                '>'
            if (SubmissionsChannel == '')
                result =
                    'Default submissions channel changed from `disabled` to <#' +
                    channel +
                    '>'
            if (channel == '')
                result =
    ...
    setMessage: {
        ...
        function: async function (bot, msg, args) {
            ...
            try {
                ...
                return (
                    'Invalid channel or message id. Could not find message ```' +
                    e +
                    '```'
                )
            }
        ...
        function: async function (bot, msg, args) {
            ...
            try {
                ...
                return (
                    ...
                    ' is now set to recieve submission updates'
                )
            } catch (e) {
    ...
    removeHost: {
        ...
        function: async function (bot, msg, args) {
                ...
                if (Host_IDs[i] == user_id) {
                    ...
                    try {
                        ...
                        return (
                            ...
                            ' (ID `' +
                            id +
                            '`) will no longer recieve submission updates'
                        )
                    } catch (e) {
                        return (
                            'ID: `' +
                            id +
                            '` will no longer recieve submission updates. Failed to send DM'
                        )
                    }
                }
            }
            ...
            return (
                'ID `' +
                user_id +
                '` Not found in list. Use `$compinfo` to check the list'
            )
        },
    },
    ...
    dqCMD: {
        ...
        function: async function (bot, msg, args) {
            ...
            let modupdate =
                `${Submissions[num].name} (${num + 1}) has been disqualified. ` +
                reason
        ...
        function: async function (bot, msg, args) {
            ...
            try {
                ...
                let get_submission_text_and_files = async (submission, i) => {
                    ...
                    let files = []
                    let filename = await submissionName(
                        bot,
                        submission.id,
                        true
                    )
                    ...
                    let filepath =
                        Save.getSavePath() + '/Submissions/' + submission.id
                    ...
                    if (submission.st.length) {
                        let st_ext = fs.existsSync(filepath + '.st')
                            ? '.st'
                            : '.savestate'
                        files.push({
                ...
                if (isNaN(args[0])) {
                    // attempt to get submission by name
                    for (let i = 0; i < Submissions.length; ++i) {
    ...
    getDownloadScript: async function (bot) {
        ...
        var addSubmission = async function (submission, dq) {
            ...
            if (
                submission.id.substr(
                    submission.id.length - 1,
            ...
            if (Nicknames[submission.id] !== undefined)
                name = Nicknames[submission.id]
            ...
            if (submission.m64)
                text += `powershell -Command "Invoke-WebRequest ${f(submission.m64)} -OutFile '${filename}.m64'\n`
            if (submission.st)
                text += `powershell -Command "Invoke-WebRequest ${f(submission.st)} -OutFile '${filename}.st'\n`
    ...
    getAllSubmissions: async function (
        bot,
        channel,
        submissions_per_zip,
        ...
        for (var i = 0; i < Submissions.length; i++) {
            ...
            if (submissions_in_zip == submissions_per_zip) {
                ...
                outputs.push(
                    fs.createWriteStream(FOLDER + zips[zips.length - 1])
                )
            ...
            for (const rf of file_extensions) {
                ...
                for (const ext of extensions) {
                    ...
                    if (fs.existsSync(filepath)) {
                        archives[archives.length - 1].file(filepath, {
                            name:
                                username +
                                '/' +
                                FilePrefix +
            ...
            if (stats.size >= 10e6) {
                ...
                if (!error_sent) {
                    await channel.createMessage(
                        'Error: zip filesize is too large. Reduce the number of submissions per zipfile.'
                    )
    ...
    listSubmissions: {
        ...
        full_descrip:
            "Shows the list of users that have submitted. This will include DQ'd submissions as well as user IDs",
        hidden: true,
        function: async function (bot, msg, args) {
            if (notAllowed(msg)) return
    ...
    info: {
        ...
        full_descrip:
            'Shows current internal variables for the competition module',
        hidden: true,
        function: async function (bot, msg, args) {
            if (notAllowed(msg)) return
    ...
    setName: {
        ...
        function: async function (bot, msg, args) {
            ...
            if (
                module.exports.hasSubmitted(user_id) &&
                (module.exports.getSubmission(user_id).submission.namelocked ||
    ...
    checkStatus: {
        ...
        full_descrip:
            "Check the status of one's submission. This tells you what you need to submit and sends you the links to your submitted files",
        hidden: true,
        function: async function (bot, msg, args) {
            try {
                var dm = await bot.getDMChannel(msg.author.id)
    ...
    setTimeWindow: {
        ...
        full_descrip:
            "Sets how long participants will have during timed tasks. Usage: `$setTaskLength <hours> <minutes>`. User's will always be given a 15 minute warning so the time cannot be less than 15 minutes.",
        hidden: true,
        function: function (bot, msg, args) {
            if (notAllowed(msg)) return

            if (args.length < 2)
                return 'Missing Arguments: `$setTaskLength <hours> <minutes>`'
            ...
            if (args[0] == 0 && args[1] < 5)
                return 'Invalid Arguments: Time must be 5 minutes minimum'
            ...
            notifyHosts(
                bot,
                `Timed tasks will now be ${Hours} hour${Hours == 1 ? '' : 's'} and ${Minutes} minutes`
            )
    ...
    setTaskMsg: {
        ...
        full_descrip:
            'Sets the task message that will be sent out to users who request the timed tasks. Everything that appears after the command call will be stored as the message. Attachments are not currently supported. To see the message use `$previewTask`',
        hidden: true,
        function: function (bot, msg, args) {
            if (notAllowed(msg)) return
            ...
            if (args.join(' ') == '') {
                ...
                result =
                    'Task message has been updated. Use `$previewTask` to view it'
            }
    ...
    previewTask: {
        ...
        full_descrip:
            'Sends you a DM with the task message to see what participants will see (admin only). To set the message use `$setTaskMsg`',
        hidden: true,
        function: async function (bot, msg, args) {
            if (notAllowed(msg)) return
    ...
    requestTask: {
        ...
        full_descrip:
            'Starts the current timed task. Once a participant calls this command they will be sent the task message (set with `$setTaskMsg`) and allowed to submit files for the allotted time (set with `$setTaskLength`). There is no confirmation, the timer will begin counting down as soon as the command is called.',
        hidden: true,
        function: function (bot, msg, args) {
            if (LateEntrants.filter((id) => id == msg.author.id).length == 0) {
                if (!TimedTask && !AllowSubmissions)
                    return `There is no active timed task to participate in right now`
                if (!TimedTask && AllowSubmissions)
                    return `You don't need to use that command right now - just send in your files!`
            }
            if (!miscfuncs.isDM(msg))
                return `This command can only be used in DMs`
            if (TimedTaskStatus.started.includes(msg.author.id))
                return `You've already started Task ${Task}!`
            if (TimedTaskStatus.completed.includes(msg.author.id))
                return `You've already completed Task ${Task}! Use \`$status\` to check your files.`

            LOG(msg.author.id, 'Started Timed Task')
            TimedTaskStatus.started.push(msg.author.id)
            TimedTaskStatus.startTimes.push([
                msg.author.id,
                timestamp(new Date()),
            ])
            module.exports.save()
            ...
            notifyHosts(
                bot,
                `${msg.author.username} \`(${msg.author.id})\` has started Task ${Task}`,
                `Timer`
            )
            bot.createMessage(msg.channel.id, TaskMessage)
            bot.createMessage(
                msg.channel.id,
                `You have started Task ${Task}. You have ${Hours} hour${Hours == 1 ? '' : 's'} and ${Minutes} minutes to submit.`
            )
        },
    },
    ...
    setLateEntrants: {
        ...
        function: function (bot, msg, args) {
            ...
            return (
                'The late entrants are now set to `' +
                LateEntrants.join(' ') +
                '`'
            )
        },
    ...
    listLateEntrants: {
        ...
        full_descrip:
            'Lists the IDs of users who can use `$requesttask` so long as they have not already participated.',
        hidden: true,
        function: function (bot, msg, args) {
            if (notAllowed(msg)) return
            if (LateEntrants.length == 0) return 'Late entrants are disabled'
    ...
    endTimedTask: async function (bot, id, notify, additionalMessage) {
        if (TimedTaskStatus.started.filter((u) => u == id).length == 0) {
            console.log(
                `Tried to end task for user that has not *started* ${id}`
            )
            return
        }
        ...
        if (RoleStyle == 'TASK-END')
            module.exports.giveRole(bot, id, user.username)
        ...
        if (notify)
            notifyHosts(
                bot,
                `Time's up for ${user.username}! ${additionalMessage}`,
                `Timer`
            )
    },
    ...
    hasSubmitted: function (user_id) {
        ...
        return (
            Submissions.filter(
                (user) =>
                    user.id == user_id ||
                    (user_id in Teams && user.id == Teams[user_id])
            ).length || DQs.filter((user) => user.id == user_id).length
        )
    },
    ...
    load: function (bot) {
        ...
        while (data.late_entrants && data.late_entrants.length > 0)
            LateEntrants.push(data.late_entrants.shift())
        Host_IDs = data.hosts
        SubmissionsChannel = data.feed
        Channel_ID = data.channel_id
        while (data.message_ids.length > 0)
            Message_IDs.push(data.message_ids.shift())
        ...
        TimedTaskStatus.started = []
        while (data.timedtaskstatus.started.length > 0)
            TimedTaskStatus.started.push(data.timedtaskstatus.started.shift())
        TimedTaskStatus.completed = []
        while (data.timedtaskstatus.completed.length > 0)
            TimedTaskStatus.completed.push(
                data.timedtaskstatus.completed.shift()
            )
        TimedTaskStatus.startTimes = []
        ...
        while (data.submissions.length > 0)
            Submissions.push(data.submissions.shift())
        while (data.dqs.length > 0) DQs.push(data.dqs.shift())
        TimeRemainingWarnings = []
        while (data.warnings.length > 0)
            TimeRemainingWarnings.push(data.warnings.shift())
        RoleStyle = data.roletype
        Object.keys(data.ignoredupdates).forEach((id) => {
            IgnoreUpdates[id] = []
            while (data.ignoredupdates[id].length)
                IgnoreUpdates[id].push(data.ignoredupdates[id].pop())
        })
        ...
        updateSubmissionMessage(bot)
        if (!fs.existsSync(Save.getSavePath() + '/Submissions')) {
            // make sure this path exists on startup
            fs.mkdirSync(Save.getSavePath() + '/Submissions')
        }
    },
    ...
    giveRole: async function (bot, user_id, name, role = SubmittedRole) {
        ...
        try {
            ...
            notifyHosts(
                bot,
                `Failed to assign ${name} (\`${user_id}\`) a role (ID: \`${role}\`)`,
                `ERROR`
            )
        }
    },
    ...
    filterSubmissions: async function (bot, msg) {
        ...
        if (!msg.attachments.some((file) => is_required_ext(fileExt(file))))
            return // contains no required files

        if (Users.isBanned(msg.author.id)) return

        if (
            !AllowSubmissions &&
            LateEntrants.filter((id) => id == msg.author.id).length == 0
        )
        ...
        if (TimedTaskStatus.completed.includes(msg.author.id)) {
            bot.createMessage(
                msg.channel.id,
                'Your time is up, you can no longer submit files. Thank you for participating.'
            )
            return
        }

        if (TimedTask && !TimedTaskStatus.started.includes(msg.author.id))
            return

        if (msg.attachments.length == 1) {
            // independent file, try to autotime (could be st or m64 change)
            filterFiles(bot, msg, msg.attachments[0], true)
            return
        }
        // download the files in order of priority, offset by 5s each time
        let total_files_to_download = msg.attachments
            .map(fileExt)
            .filter(is_required_ext).length
        ...
        let set_dl_recur = function (arr) {
            // recursively search through required files in order
            arr.forEach((ext) => {
    ...
    setReleaseDate: {
        ...
        full_descrip:
            'Usage: \`$setreleasedate <time and date...>\`\nThis sets the time for when ',
        hidden: true,
        function: function (bot, msg, args) {
            if (notAllowed(msg)) return
            ...
            if (date == null)
                return `Invalid Argument: Date not detected \`${args.join(' ')}\``

            var now = new Date()
            if (date - now < 0)
                return `Invalid Argument: Date \`${date}\` has already passed. Try a more specific time`
            ReleaseDate = date
            module.exports.save()

            result = `The task will be release publicly on \`${ReleaseDate.toString()}\`. `
            if (TimedTask) {
                // called while a task is running, need to change the time
                Announcement.KillDelayedFunction('COMP-RELEASE')
                Announcement.DelayFunction(
                    bot,
                    'COMP-RELEASE',
                    ReleaseDate.getHours() - now.getHours(),
                    ReleaseDate.getMinutes() - now.getMinutes()
                )
    ...
    releaseTask: async function (bot) {
        if (TaskChannel == ``) {
            notifyHosts(
                bot,
                `Failed to release task. No Task Channel is set. The task will not automatically stop, to do so use \`$stoptask\``,
                `ERROR`
            )
            return
        }
        try {
            await bot.createMessage(TaskChannel, {
                content: TaskMessage + `\n@everyone`,
    ...
    addTimeRemainingWarnings: function (bot, channel) {
        for (var i = 0; i < TimeRemainingWarnings.length; i++) {
            Announcement.DelayFunction(
                ...
                `COMP-WARN ${channel} ${i}`,
                Hours,
                Minutes - TimeRemainingWarnings[i]
            )
        }
    },
    ...
    timerWarning: async function (bot, channel, warningIndex) {
        ...
        try {
            await bot.createMessage(channel, {
                content: msg,
                disableEveryone: false,
            })
        } catch (e) {
            notifyHosts(
                bot,
                `Failed to give ${TimeRemainingWarnings[warningIndex]} minute warning in channel ${channel}\`\`\`${e}\`\`\``,
                `ERROR`
            )
        }
    },
    ...
    TimeRemaining: {
        ...
        function: async function (bot, msg, args) {
            ...
            if (TimedTaskStatus.completed.includes(msg.author.id))
                return `Your time is up! (0 minutes remaining)`
            if (!TimedTaskStatus.started.includes(msg.author.id))
                return `You have not started yet!`

            var startdate = TimedTaskStatus.startTimes.filter(
                (a) => a[0] == msg.author.id
            )[0][1]
    ...
    SetRoleStyle: {
        ...
        function: function (bot, msg, args) {
            ...
            if (
                args.length == 0 ||
                !ROLESTYLES.includes(args[0].toUpperCase())
            ) {
                RoleStyle = `DISABLED`
            } else {
    ...
    SetTime: {
        ...
        function: async function (bot, msg, args) {
            if (notAllowed(msg)) return
            if (args.length < 2)
                return `Missing Arguments: \`$settime <submission_number> <VIs> [additional info]\``
    ...
    AutoTime: {
        ...
        function: async function (bot, msg, args) {
                ...
                for (var i = 0; i < Submissions.length; ++i) {
                    if (
                        Submissions[i].id.substr(
                            ...
                            Submissions[i].id.length
                        ) != '-'
                    ) {
                        // [AF] ignore retiming these
            ...
            if (isNaN(args[0])) {
                // attempt to look for name in submissions
                for (let i = 0; i < Submissions.length; ++i) {
                    if (Submissions[i].name == args.join(' ')) {
                        submission_number = i
                        break
            ...
            var pos = AutoTimeEntry(
                bot,
                submission_number,
                Submissions[submission_number].id,
                -1,
                msg.channel.id,
                msg.author.id
            )
    ...
    ToggleAutoTime: {
        ...
        function: async function (bot, msg, args) {
            ...
            notifyHosts(
                bot,
                `\`Auto-timing on submission is now ${AllowAutoTime ? `enabled` : `disabled`}\``,
                `Update`
            )
        ...
        function: async function (bot, msg, args) {
            ...
            let getghost = async function (
                ...
                let queue_position = Mupen.Process(
                    ...
                    args,
                    () => {
                        if (fs.existsSync(LUAPATH + 'tmp.ghost'))
                    ...
                    async (TLE, MISMATCH_SETTINGS) => {
                        ...
                        if (TLE) {
                            ...
                            result =
                                fs
                                    .readFileSync(LUAPATH + 'error.txt')
                                    .toString() +
            ...
            let dm_ghost_data = async function (ghost_data_filename) {
                try {
                    ...
                    await dm
                        .createMessage('Here is your ghost data:', {
                            file: fs.readFileSync(LUAPATH + 'tmp.ghost'),
                            name: ghost_data_filename + '.ghost',
                        })
                        .then(fs.unlinkSync(LUAPATH + 'tmp.ghost'))
                        .catch(console.log)
                } catch (e) {
            ...
            if (notAllowed(msg)) {
                // normal users can request their own ghost
                let allowed_number = Submissions.findIndex(
                    (s) => s.id == msg.author.id
                )
                if (allowed_number == -1) {
                    return `You do not have a submission to get ghost data for!`
                } else if (
                    args.length > 0 &&
                    args[1] != allowed_number.toString()
                ) {
                    ...
                    let move_ghost = async () => {
                        ...
                        if (i == Submissions.length - 1) {
                            ...
                            try {
                                ...
                                console.log(
                                    `[ERROR] Fail to not upload zips: ${e}`
                                )
                            }
                        }
                    }
            ...
            if (args.length == 0) {
                // shortcut request their own ghost
                ghost_submission_index = Submissions.findIndex(
                    (s) => s.id == msg.author.id
                )
            ...
            if (
                ghost_submission_index < 0 ||
                Submissions.length <= ghost_submission_index
            ) {
    ...
    GetResults: {
        ...
        function: async function (bot, msg, args) {
            ...
            var result =
                args.length > 1 && Boolean(args[1])
                    ? `**__Task ${Task} Results:__**\n`
                    : ''

            var untimed = [...Submissions].filter((a) => a.time == null)
            var dqs = [...Submissions].filter((a) => a.dq)
            var timed = [...Submissions]
                .filter((a) => !a.dq && a.time != null && !a.id.endsWith('-'))
            ...
            result += `\n`
            dqs.forEach((dq) => {
                // may need to use a for loop here as well to get proper name (if a team DQs)
    ...
    IgnoreUpdate: {
        ...
        function: function (bot, msg, args) {
            ...
            if (!Host_IDs.includes(msg.author.id))
                return `Only Hosts set to receive updates can use this command. `
            if (args.length < 1)
                return `Missing Argument: \`$ignoreupdate <update>\``

            var update = args.join(` `).toUpperCase()
            if (!UPDATES.includes(update))
                return `Invalid Argument: Update must be one of: ${UPDATES.join(`, `)}`

            if (!IgnoreUpdates[msg.author.id]) IgnoreUpdates[msg.author.id] = []

            if (IgnoreUpdates[msg.author.id].includes(update)) {
                IgnoreUpdates[msg.author.id] = IgnoreUpdates[
                    msg.author.id
                ].filter((u) => u != update)
                module.exports.save()
        ...
        function: function (bot, msg, args) {
            ...
            async function send_msg(submission) {
                try {
                    ...
                    bot.createMessage(
                        msg.channel.id,
                        `Failed to broadcast message to <@${submission.id}> (${submission.id})`
                    )
                }
            }
    ...
    updateTimingScript: {
        ...
        function: function (bot, msg, args) {
            if (notAllowed(msg)) return
            var lua_files = msg.attachments.filter(
                (a) => a.filename.substr(-4).toLowerCase() == `.lua`
            )
            if (lua_files.length == 0)
                return `Missing Argument: \`$UpdateTimingScript <.lua attachment>\``
            downloadAndRun(lua_files[0], () => {
                fs.rename(
                    process.cwd() +
                        save.getSavePath().substring(1) +
                        '\\' +
                        lua_files[0].filename,
                    LUAPATH + `Conditions.lua`,
                    (err) => {
                        bot.createMessage(
                            msg.channel.id,
                            err
                                ? `Error moving file: \`\`\`${err}\`\`\``
                                : `Successfully uploaded \`Conditions.lua\``
                        )
                        notifyHosts(
                            bot,
                            `\`Conditions.lua\` has been updated by ${msg.author.username}`
                        )
                    }
                )
            })
    ...
    testTimingScript: {
        ...
        function: function (bot, msg, args) {
            ...
            let paths = [
                Save.getSavePath() + '/testconditions.m64',
                Save.getSavePath() + '/testconditions.st',
            ]
            if (lua.length > 0) {
                files.push(lua[0])
                paths.push(Save.getSavePath() + '/testconditions.lua')
            }
            Save.downloadAllFromUrl(files, paths, () => {
                let queue_pos = Mupen.Process(
                    // returns position in queue
                    bot,
                    paths[0],
                    paths[1],
                    ...
                    () => {
                        ...
                        if (lua.length > 0) {
                            // temporarily move the actual timing script
                            fs.renameSync(
                                LUAPATH + 'Conditions.lua',
                                LUAPATH + 'Conditions0.lua'
                            )
                    ...
                    async (TLE, MISMATCH_SETTINGS) => {
                        ...
                        if (TLE) {
                            result =
                                'DQ Error: time limit exceeded (the run must be timed manually).'
                        } else if (MISMATCH_SETTINGS) {
                            result = "DQ Can't playback m64."
                        } else if (fs.existsSync(LUAPATH + 'result.txt')) {
                            result = fs
                                .readFileSync(LUAPATH + 'result.txt')
                                .toString()
                            fs.unlinkSync(LUAPATH + 'result.txt')
                        } else {
                            result =
                        ...
                        if (lua.length > 0) {
                            // move the actual timing script back
                            fs.renameSync(
                                LUAPATH + 'Conditions0.lua',
                                LUAPATH + 'Conditions.lua'
                            )
                ...
                bot.createMessage(
                    msg.channel.id,
                    `Testing timing script. Position in queue: ${queue_pos}`
                )
            })
        },
    },
    ...
    toggleTeamTask: {
        ...
        function: function (bot, msg, args) {
            ...
            notifyHosts(
                bot,
                `\`Teams are now ${TeamTask ? `enabled` : `disabled`}\``,
                `Update`
            )
    ...
    setTeam: {
        ...
        function: async function (bot, msg, args) {
            ...
            if (
                team_previously_complete &&
                module.exports.hasSubmitted(msg.author.id)
            ) {
            ...
            if (partner == null && args.length)
                partner = await Users.getUser(bot, args[0])
            if (partner && partner.id == msg.author.id)
                return `You're always in a team with yourself!`

            if (
                team_previously_complete &&
                (partner == null || Teams[msg.author.id] != partner.id)
            ) {
                // changed team
                ...
                try {
                    ...
                    old_partner_dm.createMessage(
                        msg.author.username +
                            ` has removed you as their partner`
                    )
                } catch (error) {
                    bot.createMessage(
                        msg.channel.id,
                        `**Warning:** could not notify previously set partner (\`${Teams[msg.author.id]}\`) of this change.`
                    )
                }
                ...
                !team_previously_complete &&
                Teams[partner.id] == msg.author.id
            ) {
                ...
                try {
                    ...
                    bot.createMessage(
                        msg.channel.id,
                        `**Warning:** could not notify partner (\`${partner.id}\`) that your team is confirmed.`
                    )
                }
            }
            ...
            if (!team_previously_complete) {
                if (partner == null) {
                    ...
                    return (
                        `Your team is now complete! You and ` +
                        partner.username +
                        ` are set to team.`
                    )
                }
            ...
            return (
                `You have set your partner to ` +
                partner.username +
                `. To complete your team, they must add you as their teammate.`
            )
        },
    },
    ...
    setTeamName: {
        ...
        function: async function (bot, msg, args) {
            ...
            try {
                ...
                bot.createMessage(
                    msg.channel.id,
                    `Failed to notify partner of team name change`
                )
            }
    ...
    getlog: {
        ...
        function: async function (bot, msg, args) {
            ...
            msg.channel.createMessage('Here is the competition log:', {
                file: Buffer.from(result),
                name: 'SPOILER_complog.txt',
            })
        },
    },
}
