...
bot.on('ready', async () => {
    ...
    NAME = self.username
    if (!loaded) {
        // this event can fire multiple times. Don't re-register commands
        loadAllModules()
        loaded = true
    }
    ...
    if (fs.existsSync(`./crash.log`)) {
        try {
            ...
            bot.createMessage(chat.chooseChannel('bot_dms'), connected_msg, {
                ...
                .catch((err) => {
                    console.log(
                        "[Error] Failed to send 'Connected' message. " + err
                    )
                })
                .then(fs.unlinkSync(`./crash.log`))
            console.log(
                self.username + ' Ready! (' + miscfuncs.getDateTime() + ')'
            )
            return
        } catch (e) {
    ...
    bot.createMessage(chat.chooseChannel('bot_dms'), connected_msg).catch(
        (err) => {
...
function addCommand(name, func, descrip, fullDescrip, hide, aliases) {
    bot.registerCommand(
        name,
        (msg, args) => {
            if (users.isBanned(msg.author.id) && !users.hasCmdAccess(msg))
                return // disallow banned users from using any commands
    ...
    if (aliases != undefined)
        aliases.forEach((a) => bot.registerCommandAlias(a, name))
}
        ...
        if (msg.length > 1000) {
            ...
            if (cmd_number == 1) {
                addCommand(
                    mod.short_name,
                    () => message,
                    `Lists ${mod.name} commands`,
                    message,
                    false
                )
            } else {
                addCommand(
                    `${mod.short_name}${cmd_number}`,
                    () => message,
                    `Lists ${mod.name} commands`,
                    message,
                    true
...
bot.on('messageCreate', async (msg) => {
    ...
    // another meme
    if (
        msg.content.split(' ').includes('<@!532974459267710987>') ||
        msg.content.split(' ').includes('<@532974459267710987>')
    ) {
        bot.createMessage(
            msg.channel.id,
            'What the f*ck. Did you really ping me at this time for that? You did. Arrangements have been made so that I will no longer be directly pinged from you. If you need me, contact somebody else.'
        )
    }
    ...
    if (
        ...
        if (chat.chooseChannel(`flaglog`) != `flaglog`)
            bot.createMessage(
                ...
                `${user ? `${user.username}#${user.discriminator} (\`${userID}\`)` : `Unknown User`} reacted with ${emoji.name} in ${msg.channel.mention}\n> ${msg.content}\n${url}`
            )
    }

    if (
        msg.channel.guild &&
        ReactionDisabledServers.includes(msg.channel.guild.id)
    )
        return
...
var toggleReactions = function (bot, msg, args) {
    ...
    if (ReactionDisabledServers.includes(msg.channel.guild.id)) {
        ReactionDisabledServers = ReactionDisabledServers.filter(
            (id) => id != msg.channel.guild.id
        )
...
async function restart(bot, msg, args) {
    ...
    try {
        await bot.createMessage(
            msg.channel.id,
            args.length
                ? `Restarting... Downloading updated files`
                : `Restarting`
        )
    } catch (e) {
...
addCommand(
    ...
    restart,
    'Restart the bot',
    "Shuts down the bot, and starts it up again. If this is called with any arguments (ex \`$restart 1\`) it will download the latest files off of github, and then start the bot back up. This will only download files from 'bot-files'",
    true
)
addCommand(
    'tr',
    ...
    `Switches echoing reactions on/off for the current server`,
    false,
    [`toggleReaction`, `toggleReactions`]
)
...
bot.registerCommand(
    ...
    { description: 'Lists #score commands', fullDescription: score.help() }
)
bot.registerCommand(
    'log',
    (msg, args) => {
        if (users.hasCmdAccess(msg)) console.log(args.join(' '))
    },
    { hidden: true }
)
...
addCommand(
    'uptime',
    function () {
        return miscfuncs.formatSecsToStr(process.uptime())
    },
    'Prints uptime',
    'Prints how long the bot has been connected',
    false
)
...
addCommand(
    'history',
    async (bot, msg, args) => {
        if (!users.hasCmdAccess(msg) && msg.author.id != '397688653276774403')
        ...
        bot.createMessage(
            msg.channel.id,
            `Scraping file history of ${channel.mention}`
        )
        let before = channel.lastMessageID

        let iter = 0
        while (rcvd == LIMIT) {
            // uncertain about behaviour when total % LIMIT = 0
            ...
            rcvd = msgs.length
            console.log(
                `Received ${rcvd} #${iter++}. Attachments ${attachments}`
            )
            let i = 0
            for (const m of msgs) {
                ...
                for (const a of m.attachments) {
                    i--
                    let ext = a.filename
                        .substr(a.filename.lastIndexOf('.'))
                        .toLowerCase()
                    if (
                        ['.m64', '.st', '.savestate', '.zip', '.7z'].includes(
                            ext
                        )
                    ) {
                        ++attachments
        ...
        await bot.createMessage(
            ...
            [
                {
                    file: fs.readFileSync(FILEPATH + '.bat'),
                    name: '_TAS_Download.bat',
                },
                {
                    file: fs.readFileSync(FILEPATH + '.txt'),
                    name: '_TAS_Info.txt',
                },
            ]
        )
