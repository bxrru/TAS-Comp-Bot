...
module.exports = {
    ...
    formatSecsToStr: (sec) => {
        ...
        const seconds = (sec - minutes * 60 - hours * 60 * 60).toFixed(3)
    ...
    ping: {
        ...
        short_descrip: 'ping',
        full_descrip:
            'To check if the bot is not dead. Tells you time it takes to bait you in ms',
        hidden: true,
        function: (bot, msg) =>
            `baited (${(Date.now() - msg.timestamp) / 1000}ms)`,
    },
        ...
        function: async (bot, msg, args) => {
            ...
            try {
                await bot.addGuildMemberRole(
                    ...
                    member,
                    args[0],
                    `Command Call by ${msg.author.username}`
                )
    ...
    addReaction: {
        ...
        full_descrip:
            'Usage `$react <channel_id> <message_id> <emojis...>`\nThis will reacat with multiple space separated emojis. For a list of channel names that can be used instead of `<channel_id>` use `$lc`',
        hidden: true, // T?
        function: async (bot, msg, args) => {
            if (!users.hasCmdAccess(msg)) return
            if (args.length < 3)
                return 'Not Enough Arguments: `<channel_id> <message_id> <emojis...>`'

            const channel = args.shift()
            const message = args.shift()

            for (let i = 0; i < args.length; i++)
                if (args[i].includes(':')) args[i] = args[i].substr(2, -3)

            args.forEach((emoji) => {
                bot.addMessageReaction(
                    chat.chooseChannel(channel),
                    message,
                    emoji
                ).catch((e) => {
    ...
    getUserID: (arg) =>
        arg.startsWith('<@') && arg.endsWith('>') ? arg.substr(2, -3) : arg,
